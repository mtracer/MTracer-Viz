...
%>
<a href="index.jsp">Return Home</a>&gt;&gt;<a href="index.jsp">Tasks</a>&gt;&gt;<a href="callTreeGraph.jsp?taskid=<%= taskID %>&delay=<%= delay%>&direction=TB&shape=ellipse">CTG</a> 
	...
	for(int i=0; i<sortedReports.size();i++)
	{
		...
		for(int s=0;s<depth;s++)
			for(int t=0;t<16;t++)
				space+="&nbsp;";
		if(depth==0)
			out.print("<td>"+space+"<a title = \"see all tasks of this type\" href=\"index.jsp?title="+opname+"&numReportsFrom="+sortedReports.size()+"&numReportsTo="+sortedReports.size()+"\">"+formattedOpname+"</td>");
		else
