...
<a href="index.jsp">Return Home</a> &gt;&gt;<a href="index.jsp">Tasks</a>&gt;&gt;<B>Task classify</B> | <a href="op_index.jsp">Operations</a><hr/>
</div>
...
for (Iterator it = topoids.iterator(); it.hasNext();) {
	...
	//out.print("<td align = center><a title = \"see all tasks of this type\" href=\"index.jsp?title="+oldTitle+"&taskID="+taskIDs+"\">" + tasks.size() + "</a></td>");
	//url不能太长，所以taskids不能通过url传送，而将其存储在session中，告诉下一个页面通过该session访问
	out.print("<td align = center><a onClick = \"storeTaskIds(); return false;\" title = \"see all tasks of this type\" href=\"index.jsp?title="+oldTitle+"&sortValue=delay&sortStyle=descend&taskID=usesession&sessionKey="+topoid+"\">" + tasks.size() + "</a></td>");
	String tmp = "";
